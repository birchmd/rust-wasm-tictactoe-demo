// Taken from https://github.com/paritytech/wasmi/blob/master/examples/tictactoe.rs
//
// This file defines the game engine. It takes wasm programs as inputs, which act as
// the players in the game. This is accomplished by describing an API between the engine
// and the wasm programs. The programs must export `mk_turn()`, which is then called
// when it is that players turn. The programs must import `get(idx: i32) -> i32` and
// `set(idx: i32)` which they can then call within `mk_turn` to interact with the
// board.
...
fn main() {
    ...
    if args.len() < 3 {
        println!("Usage: {} <X player module> <O player module>", args[0]);
        return;
    }

    // Instantiate modules of X and O players.
    let x_instance = instantiate(&args[1]).expect("X player module to load");
    let o_instance = instantiate(&args[2]).expect("O player module to load");
